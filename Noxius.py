...
os.system('cls')
os.system("title NOXIUS TM CORPORATION ^| Bienvenido a Noxius Doxxer ^| Made By SadicX And Fran ^|")
...
gui = """
                    ...
                    ╔═══════════════════════════════════════════════════════╗ ╔═══════════════╗
                    ║  d8b   db  .d88b.  db    db d888888b db    db .d8888. ║ ║    MADE BY    ║
                    ║  888o  88 .8P  Y8. `8b  d8'   `88'   88    88 88'  YP ║ ║ FRAN & SADICX ║                
                    ║  88V8o 88 88    88  `8bd8'     88    88    88 `8bo.   ║ ╠═══════════════╣
                    ║  88 V8o88 88    88  .dPYb.     88    88    88   `Y8b. ║ ║  VERSION: 3.0 ║
                    ║  88  V888 `8b  d8' .8P  Y8.   .88.   88b  d88 db   8D ║ ╠═══════════════╣
                    ║  VP   V8P  `Y88P'  YP    YP Y888888P ~Y8888P' `8888Y' ║ ║  - UPDATES -  ║
                    ║             https://github.com/Noxius-TM              ║ ║ [+] NEW GUI   ║
                    ╚════╦═══════════════════════════════════════════╦══════╝ ╚═╦═══════════╦═╝
                         ║                                           ║          ║           ║     
                       ╔═╩═══════════════════════════════════════════╩═╗ ╔══════╩═══════════╩══════╗
                       ║      [ https://discord.gg/FNxxXXKYJV ]        ║ ║  LETS DESTROY THE WORLD ║
                       ╚═══════════════════════════════════════════════╝ ╚═════════════════════════╝
"""
faded_text = fade.brazil(gui)
print(faded_text)

username = input(Fore.WHITE + '                       [!] Introduce el nombre: ')
os.system(f"title Doxxeando a {username} ^| Tiempo estimado: 1 Minuto ^| LETS DESTROY THE WORLD")
...
def search():
    ...
    faded_text = fade.brazil(gui)
    print(faded_text)
    print(f'                                   [+] Buscar por nombre de usuario:{username} [+]')
    time.sleep(1)
    os.system("cls")
    faded_text = fade.brazil(gui)
    print(faded_text)    
    ...
    for url in WEBSITES:
        ...
        if r.status_code == 200:
            if match == True:
                ...
                match = False
            YELLOW(Fore.LIGHTBLACK_EX + f'\n                             [ + ] - {url} - {r.status_code} - OK - [ + ]')
            if username in r.text:
                print(Fore.LIGHTCYAN_EX + f'                                    {locijl}', Fore.LIGHTGREEN_EX + f'{username} --> L I V E <--')
            else:
                print(Fore.LIGHTCYAN_EX + f'                                    {locijl}', Fore.LIGHTRED_EX + f'{username} --> D E A D <--')
        count += 1
    ...
    faded_text = fade.brazil(gui)
    print(faded_text)
    print(Fore.LIGHTCYAN_EX + f' -> FIN! - Se han detectado {count} Cuentas. | En {total} Paginas distintas.')
